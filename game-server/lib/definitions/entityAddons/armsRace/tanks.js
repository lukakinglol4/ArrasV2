...
// ═══════════════════════════════════════════════════════════════════════
// NEW TANKS — Arras Mobile Custom Addition
// ═══════════════════════════════════════════════════════════════════════

// ── Tier 4+ unique tanks ────────────────────────────────────────────────

// OBLITERATOR — mega destroyer with a front cannon and side auto-turrets
Class.obliterator_AR = {
    PARENT: 'genericTank',
    LABEL: 'Obliterator',
    UPGRADE_COLOR: 'orange',
    UPGRADE_TOOLTIP: 'Fires a massive shell with twin auto-turrets for coverage.',
    SHAPE: 1,
    GUNS: [
        { // main cannon
            POSITION: [22, 16, 1, 0, 0, 0, 0],
            PROPERTIES: {
                SHOOT_SETTINGS: combineStats([g.basic, g.pounder, g.destroyer, {reload: 1.4, recoil: 2.5, size: 1.5}]),
                TYPE: 'bullet',
            }
        },
        { // left flanker
            POSITION: [9, 5, 1, 0, -5, -90, 0.5],
            PROPERTIES: {
                SHOOT_SETTINGS: combineStats([g.basic, g.machineGun]),
                TYPE: 'bullet',
                AUTOFIRE: true,
...
if (Config.arms_race) {
    Class.destroyer.UPGRADES_TIER_3 = Class.destroyer.UPGRADES_TIER_3 || [];
    Class.destroyer.UPGRADES_TIER_3.push('obliterator_AR');
}

// PHANTOM — stealth sniper that fades while stationary
Class.phantom_AR = {
    PARENT: 'genericTank',
    LABEL: 'Phantom',
    UPGRADE_COLOR: 'purple',
    UPGRADE_TOOLTIP: 'A long-range sniper. Becomes nearly invisible when still.',
    SHAPE: 1,
    GUNS: [
        {
            POSITION: [28, 7, 1, 0, 0, 0, 0],
            PROPERTIES: {
                SHOOT_SETTINGS: combineStats([g.basic, g.sniper, g.assassin, g.ranger, {speed: 1.4, range: 1.5, damage: 1.6, reload: 0.55}]),
                TYPE: 'bullet',
            }
        }
    ],
...
if (Config.arms_race) {
    Class.ranger.UPGRADES_TIER_4 = Class.ranger.UPGRADES_TIER_4 || [];
    Class.ranger.UPGRADES_TIER_4.push('phantom_AR');
}

// STORMCALLER — rapid-fire tri-barrel with storm drones
Class.stormcaller_AR = {
    PARENT: 'genericTank',
    LABEL: 'Stormcaller',
    UPGRADE_COLOR: 'blue',
    UPGRADE_TOOLTIP: 'Triple barrels and storm drones make it a menace at mid range.',
    SHAPE: 1,
    GUNS: [
        { POSITION: [18, 7, 1, 0, 0, 0, 0], PROPERTIES: { SHOOT_SETTINGS: combineStats([g.basic, g.tripleShot, {reload: 1.2, speed: 1.1}]), TYPE: 'bullet' } },
        { POSITION: [16, 7, 1, 0, -4, -12, 0.5], PROPERTIES: { SHOOT_SETTINGS: combineStats([g.basic, g.tripleShot, {reload: 1.2, speed: 1.1}]), TYPE: 'bullet' } },
        { POSITION: [16, 7, 1, 0, 4, 12, 0.5], PROPERTIES: { SHOOT_SETTINGS: combineStats([g.basic, g.tripleShot, {reload: 1.2, speed: 1.1}]), TYPE: 'bullet' } },
        { POSITION: [7, 7, 1.5, 7, 0, 180, 0], PROPERTIES: { SHOOT_SETTINGS: combineStats([g.swarm, {reload: 1.4, health: 1.1}]), TYPE: 'autoswarm', STAT_CALCULATOR: 'swarm', AUTOFIRE: true, MAX_CHILDREN: 4 } },
    ]
}
if (Config.arms_race) {
    Class.tripleShot.UPGRADES_TIER_3 = Class.tripleShot.UPGRADES_TIER_3 || [];
    Class.tripleShot.UPGRADES_TIER_3.push('stormcaller_AR');
}

// TEMPLAR — heavily armoured rammer/tank hybrid
Class.templar_AR = {
    PARENT: 'genericTank',
    LABEL: 'Templar',
    UPGRADE_COLOR: 'gold',
    UPGRADE_TOOLTIP: 'Enormous body damage and health. Wins fights by running into things.',
    SHAPE: 8,
    SIZE: 1.2,
    GUNS: [
        { POSITION: [10, 10, 1, 0, 0, 0, 0], PROPERTIES: { SHOOT_SETTINGS: combineStats([g.basic, g.pounder, {damage: 1.2, reload: 0.8}]), TYPE: 'bullet' } },
    ],
    BODY: {
        SPEED: preset.SPEED * 1.2,
        HEALTH: preset.HEALTH * 2,
        DAMAGE: preset.DAMAGE * 4,
        SHIELD: preset.SHIELD * 1.5,
    }
}
if (Config.arms_race) {
    Class.smasher.UPGRADES_TIER_3 = Class.smasher.UPGRADES_TIER_3 || [];
    Class.smasher.UPGRADES_TIER_3.push('templar_AR');
}

// HELIX DRONE — director variant that fires spinning drone clusters
Class.helixdrone_AR = {
    PARENT: 'genericTank',
    LABEL: 'Helix Drone',
    UPGRADE_COLOR: 'teal',
    UPGRADE_TOOLTIP: 'Launches spiralling drone clusters that orbit enemies.',
    SHAPE: 1,
    GUNS: [
        ...weaponArray({
            POSITION: [8, 7, 1.4, 7, 0, 0, 0],
            PROPERTIES: {
                SHOOT_SETTINGS: combineStats([g.drone, g.overseer, {reload: 1.5, health: 1.2, damage: 1.1}]),
...
if (Config.arms_race) {
    Class.overseer.UPGRADES_TIER_3 = Class.overseer.UPGRADES_TIER_3 || [];
    Class.overseer.UPGRADES_TIER_3.push('helixdrone_AR');
}

// NOVA — self-destructing bullet that creates an explosion ring
Class.nova_AR = {
    PARENT: 'genericTank',
    LABEL: 'Nova',
    UPGRADE_COLOR: 'red',
    UPGRADE_TOOLTIP: 'Fires explosive cluster shots that split on impact.',
    SHAPE: 1,
    GUNS: [
        {
            POSITION: [18, 11, 1, 0, 0, 0, 0],
            PROPERTIES: {
                SHOOT_SETTINGS: combineStats([g.basic, g.pounder, {reload: 0.7, damage: 1.4, size: 1.3, speed: 0.85}]),
                TYPE: ['hive', {INDEPENDENT: true}],
            }
        }
    ]
}
if (Config.arms_race) {
    Class.artillery.UPGRADES_TIER_3 = Class.artillery.UPGRADES_TIER_3 || [];
    Class.artillery.UPGRADES_TIER_3.push('nova_AR');
}
