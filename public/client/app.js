...
(async function (util, global, config, Canvas, color, gameDraw, socketStuff) {
    ...
    // Smooth visual particles for hits, deaths, and level-ups.
    const particles = [];

    function spawnParticles(x, y, count, colorStr, speed = 4, size = 4, life = 30) {
    ...
    function updateAndDrawParticles(ctx2, ratio) {
        ...
        for (let i = particles.length - 1; i >= 0; i--) {
            ...
            // World → screen
            const sx = (p.x - px) / ratio + sw / 2;
            const sy = (p.y - py) / ratio + sh / 2;

            ctx2.globalAlpha = p.alpha * 0.85;
            ctx2.fillStyle = p.color;
            ctx2.beginPath();
            ctx2.arc(sx, sy, Math.max(0.5, p.size * p.alpha), 0, Math.PI * 2);
            ctx2.fill();
        }
        ctx2.globalAlpha = 1;
    }

    // Hook entity death to spawn death particles
    const _origEntityDie = global.onEntityDie;
    global.onEntityDie = (x, y, colorStr) => {
        spawnParticles(x, y, 18, colorStr || '#ffffff', 5, 5, 40);
        if (_origEntityDie) _origEntityDie(x, y, colorStr);
    };

    // Hook hit events to spawn small hit sparks
    global.onEntityHit = (x, y, colorStr) => {
        spawnParticles(x, y, 5, colorStr || '#ffcc00', 2.5, 2.5, 15);
    };

    // Level-up burst
    let _lastLevel = 0;
    function checkLevelUp(tick) {
        const lv = gui.__s.getLevel();
        if (lv > _lastLevel && _lastLevel > 0) {
            const px = global.player.renderx;
            const py = global.player.rendery;
            spawnParticles(px, py, 30, '#00ffcc', 7, 6, 50);
        }
        _lastLevel = lv;
    }
    // ─────────────────────────────────────────────────────────────────────
    ...
    function animloop(tick) {
        ...
        try {
            ...
            updateAndDrawParticles(ctx[1], ratio);
